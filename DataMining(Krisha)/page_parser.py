...
def parse_html_and_save(url, headers, cookies):
    try:
        ...
        if "Read timed out" in str(e):
            print(f"❌ Таймаут на {url}, прерываем выполнение и сохраняем прогресс.")
            with open(PROGRESS_FILE, "w") as pf:
                json.dump({"last_url": url}, pf)
            raise e
        with log_lock:
...
if os.path.exists(PROGRESS_FILE):
    with open(PROGRESS_FILE, "r") as f:
        try:
            ...
            if last_url and last_url in example_urls:
                idx = example_urls.index(last_url)
                example_urls = example_urls[idx:]
        except Exception:
            pass
...
url_chunks = list(chunkify(example_urls, 30))
